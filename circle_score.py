...
def pixelIterate(img, circle, index):
    ...
    for y in range(max(circle[1]-circle[2],0),min(circle[1]+circle[2],h )): # type: ignore
        for x in range(max(circle[0]-circle[2],0),min(circle[0]+circle[2],w)): # type: ignore
...
def get_miroslav_score(img, circles):
...
def get_barish_score(img, circles):
...
def getCompositeScore(img, circles):
    ...
    return (get_barish_score(img_dilation, circles),get_miroslav_score(img, circles))
...
def filter_by_composite_score(img, circles, b_score_threshold, m_score_threshold):
    ...
    retListFinal=[]
    retList=[]
    bScore = get_barish_score(img, circles)
    #print(bScore)

    for i in range(0, len(circles)):
        if  bScore[i] <=b_score_threshold:
            retList+=[circles[i]]

    mScore = get_miroslav_score(img, retList)
    #print(mScore)
    for i in range(0, len(retList)):
        if  mScore[i] >= m_score_threshold:
